...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 19, 2022 at 01:06 PM
...
CREATE TABLE `matakuliahs` (
  `id` varchar(5) COLLATE utf8mb4_unicode_ci NOT NULL,
  `kode_prodi` varchar(3) COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `matakuliahs` (`id`, `kode_prodi`, `nama_matakuliah`, `kategori_matakuliah`, `bentuk_matakuliah`, `kuota`, `sks`, `semester`, `created_at`, `updated_at`) VALUES
...
INSERT INTO `users` (`nip`, `kode_prodi`, `nama`, `jabatan`, `foto`, `email`, `email_verified_at`, `password`, `status`, `remember_token`, `created_at`, `updated_at`) VALUES
...
ALTER TABLE `matakuliahs`
  ADD PRIMARY KEY (`id`),
  ADD KEY `matakuliahs_kode_prodi_foreign` (`kode_prodi`);
...
ALTER TABLE `users`
  ...
  ADD KEY `users_kode_prodi_foreign` (`kode_prodi`);
...
--
ALTER TABLE `matakuliahs`
  ADD CONSTRAINT `matakuliahs_kode_prodi_foreign` FOREIGN KEY (`kode_prodi`) REFERENCES `prodis` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `users`
  ADD CONSTRAINT `users_kode_prodi_foreign` FOREIGN KEY (`kode_prodi`) REFERENCES `prodis` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
